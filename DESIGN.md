...
7. **shadcn-vue 落地映射**(更新 `web/src/assets/main.css` 的 `:root` 块,**不需要保留 `.dark`** —— 这套设计是 dark-first,亮色版本见文末「附录：Light 变体」):
...
> 本设计体系为 dark-first，暗色是其原生与默认形态。此附录为需要「明暗双主题 / 跟随系统」的产品补充一套对应的浅色调色板。它**扩展**核心语言而非替代它：几何（pill 按钮、12px 卡片、6px 输入）、字体、间距、阴影规则、圆角标度全部不变，仅切换颜色层。

### 设计原则（Light）

- **反转表面层级，不反转语义**：暗色用「越靠前越亮」的四级灰阶抬升；浅色改为「白底 + 四级浅灰阶」，方向相反但层级语义一致（raised → card → hover）。
- **主色与 CTA 不变**：emerald `#22c55e` 仍是唯一主色；CTA 仍绿底黑字（`#000` on `#22c55e`，浅底对比度同样 5.6:1，AA 通过）。
- **强调文字色为浅底重新定深**：暗底上偏亮的强调色（如 `primary-soft #4ade80`）在白底会过浅，故浅色变体把「用作文字的强调 / 语义色」整体下沉一档以保证 AA；用作填充 / 圆点的主色与成功色保持不变。
- **边线方向反转**：暗色 hairline 接近黑（`#1a`）；浅色 hairline 接近白（`#ece`）。
- **阴影规则不变**：浅色下静态卡片同样不加 shadow，仅靠 `edge-subtle` 与底色区分；浮层 shadow 可酌情减淡。

### Surface（浅色四级灰阶）

| Level | Light | 对应暗色 | Use |
|---|---|---|---|
| 0 / Base | `#ffffff` | `#000000` | body |
| 1 / Raised | `#f7f7f8` | `#0a0a0a` | 次级面板 / Input 内底 |
| 2 / Card | `#ffffff` | `#111111` | 标准卡片 |
| 3 / Hover | `#f4f4f5` | `#181818` | hover 态 |
| 4 / Overlay | `#ffffff` | `#1c1c1f` | 浮层（配 shadow） |

### Edge（浅色）

| Token | Light | 对应暗色 |
|---|---|---|
| edge-subtle | `#ececee` | `#1a1a1a` |
| edge | `#e4e4e7` | `#222222` |
| edge-strong | `#d4d4d8` | `#2a2a2e` |
...
| ink-mute | `#71717a` | `#8a8a97` |
| ink-disabled | `#a1a1aa` | `#56565f` |

### 强调 / 语义（浅色重新定深）

| 角色 | Light | 暗色 | 说明 |
|---|---|---|---|
| primary（CTA 填充） | `#22c55e` | `#22c55e` | 不变 |
| on-primary（CTA 文字） | `#000000` | `#000000` | 不变，绿底黑字 |
| primary-soft（链接 / 正向文字） | `#15803d` | `#4ade80` | 浅底下沉保证可读 |
| success（状态圆点） | `#22c55e` | `#22c55e` | 不变 |
| danger | `#dc2626` | `#ef4444` | 浅底略深 |
| warning（文字） | `#b45309` | `#f59e0b` | 浅底下沉 |
| info（文字） | `#1d4ed8` | `#3b82f6` | 浅底下沉 |

> 软徽章规则不变：`语义色 12% 透明背景 + 100% 文字`。浅色下文字取上表 Light 值即可在浅底保持 AA。

### 双主题落地（Tailwind v4 + shadcn，class 切换）

`:root` 放浅色、`.dark` 放暗色（仅覆盖差异项）；`@theme inline` 以 `var(--x)` 引用令牌，随 `<html>` 的 `light` / `dark` class 自动切换。品牌恒定令牌（`primary` / `primary-foreground` / `ring` / `success` / 图表色）只写在 `:root`，由 `.dark` 继承。

```css
:root {
  /* Light：白底 + 四级浅灰阶 + 深 ink */
  --background:#ffffff; --foreground:#18181b;
  --card:#ffffff; --card-foreground:#18181b;
  --popover:#ffffff; --popover-foreground:#18181b;
  --primary:#22c55e; --primary-foreground:#000000;
  --secondary:#f4f4f5; --secondary-foreground:#18181b;
  --muted:#f4f4f5; --muted-foreground:#52525b;
  --accent:#f4f4f5; --accent-foreground:#18181b;
  --destructive:#dc2626;
  --border:#e4e4e7; --input:#d4d4d8; --ring:#22c55e;
  --surface-raised:#f7f7f8; --surface-card:#ffffff; --surface-hover:#f4f4f5; --surface-overlay:#ffffff;
  --edge-subtle:#ececee; --edge:#e4e4e7; --edge-strong:#d4d4d8;
  --ink-primary:#18181b; --ink-secondary:#52525b; --ink-mute:#71717a; --ink-disabled:#a1a1aa;
  --primary-soft:#15803d; --primary-deep:#16a34a; --primary-press:#15803d;
  --success:#22c55e; --warning:#b45309; --info:#1d4ed8;
}

.dark {
  /* Dark：本体系原生形态，仅列与 :root 的差异项 */
  --background:#000000; --foreground:#ededef;
  --card:#111111; --card-foreground:#ededef;
  --popover:#1c1c1f; --popover-foreground:#ededef;
  --secondary:#111111; --secondary-foreground:#ededef;
  --muted:#0a0a0a; --muted-foreground:#a1a1aa;
  --accent:#181818; --accent-foreground:#ededef;
  --destructive:#ef4444; --border:#1a1a1a; --input:#222222;
  --surface-raised:#0a0a0a; --surface-card:#111111; --surface-hover:#181818; --surface-overlay:#1c1c1f;
  --edge-subtle:#1a1a1a; --edge:#222222; --edge-strong:#2a2a2e;
  --ink-primary:#ededef; --ink-secondary:#a1a1aa; --ink-mute:#8a8a97; --ink-disabled:#56565f;
  --primary-soft:#4ade80; --warning:#f59e0b; --info:#3b82f6;
}
```

> 跟随系统：用支持 `class` 策略的主题方案在 `<html>` 上写入 `light` / `dark`，上面两套 token 即自动生效。CTA 文字在两套主题下均强制 `on-primary`（黑色），不用 `text-white`。
