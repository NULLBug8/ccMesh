...
pub fn run_migrations(conn: &Connection) -> AppResult<()> {
    ...
    for (index, script) in MIGRATIONS.iter().enumerate() {
        ...
        if version > current {
            execute_migration_script(conn, script)?;
...
fn ensure_column(conn: &Connection, table: &str, column: &str, ddl: &str) -> AppResult<()> {
    if !table_has_column(conn, table, column)? {
        ...
        tracing::warn!(table, column, "补齐缺失数据库列");
    }
    Ok(())
}

fn ensure_required_columns(conn: &Connection) -> AppResult<()> {
    ensure_column(
        conn,
        "request_logs",
        "inbound_path",
...
mod tests {
    ...
    #[test]
    fn migration_recovers_when_schema_version_lags_existing_column() {
        let conn = Connection::open_in_memory().unwrap();
        run_migrations(&conn).unwrap();
        conn.execute(
    ...
    #[test]
    fn migration_recovers_when_schema_version_is_current_but_request_log_column_is_missing() {
        let conn = Connection::open_in_memory().unwrap();
        conn.execute_batch(
            ...
            CREATE TABLE endpoints (
                ...
                balance_query TEXT NOT NULL DEFAULT '{}',
                remark TEXT NOT NULL DEFAULT '',
                sort_order INTEGER NOT NULL DEFAULT 0,
                test_status TEXT NOT NULL DEFAULT 'unknown',
                created_at TEXT NOT NULL DEFAULT (datetime('now')),
