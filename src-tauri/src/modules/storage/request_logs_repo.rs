...
fn row_to_log(row: &rusqlite::Row) -> rusqlite::Result<RequestLog> {
    let trace_json: Option<String> = row.get(18)?;
    ...
    Ok(RequestLog {
        ...
        inbound_path: row.get(13)?,
        upstream_path: row.get(14)?,
        first_byte_ms: row.get(15)?,
        actual_model: row.get(16)?,
        error_body: row.get(17)?,
        trace,
    })
}
