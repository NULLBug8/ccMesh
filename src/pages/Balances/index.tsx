...
function BalanceSummary({ result }: { result?: EndpointBalanceResult }) {
  ...
  const hasBalance = result.balance !== null && result.balance !== undefined;
  return (
    <div className="space-y-1">
      {hasBalance ? (
        <span className="inline-flex rounded-full bg-success/10 px-2 py-1 text-sm font-medium text-success">
          {fmtBalanceValue(result.balance)}
          {result.currency ? ` ${result.currency}` : ""}
        </span>
      ) : result.used ? (
        <span className="inline-flex rounded-full bg-warning/10 px-2 py-1 text-sm font-medium text-warning">
          已用 {fmtBalanceValue(result.used)}
        </span>
      ) : null}
        ...
        <div className="space-y-0.5 text-xs text-ink-mute">
          {result.limits.map((limit) => (
            <div key={limit.label}>
              ...
              {limit.used ? ` / 已用 ${fmtBalanceValue(limit.used)}` : ""}
...
export function Balances() {
  ...
  const { results, setResults } = useEndpointBalanceResults();
