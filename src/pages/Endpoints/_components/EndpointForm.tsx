...
function uniqueBalanceProbeResults(results: BalanceProbeTemplateResult[]) {
  const byPath = new Map<string, BalanceProbeTemplateResult>();
  for (const item of results) {
    const key = item.path;
    const current = byPath.get(key);
    if (
      !current ||
      Number(item.success) > Number(current.success) ||
      (item.urlReachable && !current.urlReachable)
    ) {
      byPath.set(key, item);
    }
  }
...
export function EndpointForm({ open, onOpenChange, editing }: Props) {
  ...
  const visibleBalanceProbeResults = balanceProbe
    ? uniqueBalanceProbeResults(balanceProbe.results)
    : [];

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        data-testid="endpoint-editor-workspace"
        className="!flex !h-[calc(100dvh-1rem)] !w-[calc(100vw-1rem)] !max-w-[calc(100vw-1rem)] flex-col gap-0 !overflow-hidden rounded-2xl border-edge bg-background p-0 sm:!max-w-[calc(100vw-1rem)]"
      >
        <DialogHeader className="shrink-0 border-b border-edge bg-surface/90 px-6 py-4">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <DialogTitle className="mt-1 text-xl">
                {editing ? `编辑端点 · ${editing.name}` : "新建端点"}
              </DialogTitle>
              <p className="mt-1 text-sm text-ink-mute">
                基础连接、模型公布、余额模板和 JSON 配置集中在同一个工作区。
              </p>
            </div>
            <div className="grid grid-cols-3 gap-2 text-xs">
              <div className="rounded-lg border border-edge-subtle bg-background/70 px-3 py-2">
                <div className="text-ink-mute">类型</div>
                <div className="mt-1 font-medium">{form.transformer}</div>
              </div>
              <div className="rounded-lg border border-edge-subtle bg-background/70 px-3 py-2">
                <div className="text-ink-mute">模型</div>
                <div className="mt-1 font-medium tabular-nums">{form.models.length}</div>
              </div>
              <div className="rounded-lg border border-edge-subtle bg-background/70 px-3 py-2">
                <div className="text-ink-mute">余额</div>
                <div className="mt-1 font-medium">{form.balanceQuery.enabled ? "启用" : "关闭"}</div>
              </div>
            </div>
          </div>
        </DialogHeader>

        <Tabs value={tab} onValueChange={setTab} className="flex min-h-0 flex-1 flex-col overflow-hidden px-6 py-4">
          <TabsList className="shrink-0 self-start rounded-xl bg-surface-raised">
            <TabsTrigger value="form">表单</TabsTrigger>
            <TabsTrigger value="balance">余额</TabsTrigger>
            <TabsTrigger value="json">JSON</TabsTrigger>
          </TabsList>

          <TabsContent value="form" className="min-h-0 flex-1 overflow-y-auto pr-2 pt-4">
            <div className="grid min-h-full gap-4 xl:grid-cols-[minmax(520px,0.9fr)_minmax(520px,1.1fr)]">
              <section className="rounded-2xl border border-edge bg-surface/80 p-4">
                <div className="mb-4">
                  <h3 className="text-sm font-semibold">连接信息</h3>
                  <p className="mt-1 text-xs text-ink-mute">
                    填写上游根地址、密钥和转换协议。URL 只填根域名，不要手动追加 /v1。
                  </p>
                </div>
            ...
            {fields.map((f) => (
              ...
              <section className="flex min-h-0 flex-col gap-4 rounded-2xl border border-edge bg-surface/80 p-4">
                <div>
                  <h3 className="text-sm font-semibold">协议与模型</h3>
                  <p className="mt-1 text-xs text-ink-mute">
                    转换器决定请求路径；模型清单决定对外公布和测试弹窗候选。
                  </p>
                </div>
            ...
            <div className="flex min-h-0 flex-col gap-1.5">
              ...
              {form.models.length > 0 && (
                <>
                  <div className="flex max-h-72 flex-wrap gap-1.5 overflow-auto rounded-md border border-edge bg-background/50 p-2">
                    {form.models.map((m) => {
                      const lit = isLit(m);
                      return (
                        <Badge
          ...
          <TabsContent value="balance" className="min-h-0 flex-1 overflow-y-auto pr-2">
            <div className="grid min-h-full gap-4 xl:grid-cols-[minmax(520px,0.95fr)_minmax(620px,1.05fr)]">
              ...
              {balanceProbe ? (
                <div className="mt-4 space-y-3">
                  ...
                  <div className="overflow-hidden rounded-lg border border-edge-subtle">
                    ...
                    {visibleBalanceProbeResults.map((item) => (
                      <div
            ...
            <div className="flex flex-col gap-4 rounded-xl border border-edge bg-surface/80 p-4">
            ...
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="balance-query-body-template">请求体模板</Label>
              <textarea
                id="balance-query-body-template"
                value={form.balanceQuery.body}
                placeholder='{"user":"{{endpointName}}"}'
                onChange={(e) => updateBalance({ body: e.target.value, templateId: "custom" })}
                className="min-h-40 w-full resize-y rounded-sm border border-input bg-surface-raised px-3 py-2 font-mono text-sm leading-6 outline-none transition-[color,box-shadow] focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 disabled:cursor-not-allowed disabled:opacity-50"
              />
              <p className="text-xs text-ink-mute">
          ...
          <TabsContent value="json" className="min-h-0 flex-1 w-full min-w-0 overflow-hidden">
            {tab === "json" ? (
              <Suspense
                fallback={
        ...
        <DialogFooter className="shrink-0 border-t border-edge-subtle pt-3">
