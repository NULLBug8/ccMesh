...
describe("RequestTracePanel", () => {
  ...
  it("uses response header labels for upstream and client response stages", () => {
    render(
      <RequestTracePanel
        log={{
          ...tracedLog,
          trace: {
            ...tracedLog.trace!,
            receivedForwardedRequest: {
              ...tracedLog.trace!.receivedForwardedRequest,
              headers: [],
            },
            responseRequest: {
              ...tracedLog.trace!.responseRequest,
              headers: [],
            },
          },
        }}
    ...
    expect(screen.getAllByText("请求头")).toHaveLength(2);
    expect(screen.getAllByText("响应头")).toHaveLength(2);
    expect(screen.queryByText("无请求头")).not.toBeInTheDocument();
    expect(screen.getAllByText("无响应头")).toHaveLength(2);
  });

  it("wraps body previews instead of forcing a single horizontal line", () => {
    const { container } = render(<RequestTracePanel log={tracedLog} />);

    const bodyPreview = container.querySelector("pre");
    expect(bodyPreview).toHaveClass("whitespace-pre-wrap");
    expect(bodyPreview).toHaveClass("break-words");
  });

  it("renders trace stages as a single readable timeline", () => {
    const { container } = render(<RequestTracePanel log={tracedLog} />);

    expect(screen.getByTestId("request-trace-timeline")).toBeInTheDocument();
    const stageCards = container.querySelectorAll("[data-testid='request-trace-stage']");
    expect(stageCards).toHaveLength(4);
    stageCards.forEach((card) => {
      expect(card).toHaveClass("grid-cols-[minmax(0,1fr)]");
    });
  });
