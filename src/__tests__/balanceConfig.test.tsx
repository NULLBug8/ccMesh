...
describe("Endpoint balance template assistant", () => {
  it("renders endpoint editing as a wide workspace and keeps request body template editable", async () => {
    renderEndpointForm();
    ...
    const bodyTemplate = screen.getByLabelText("请求体模板");
    expect(bodyTemplate.tagName).toBe("TEXTAREA");
    expect(bodyTemplate).toHaveClass("min-h-40");
  });
  ...
  it("deduplicates balance probe rows by URL path", async () => {
    vi.spyOn(endpointApi, "probeBalanceTemplates").mockResolvedValueOnce({
    ...
    expect(await screen.findAllByText("/api/user/self")).toHaveLength(1);
  });
