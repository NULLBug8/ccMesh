...
describe("Endpoints layout", () => {
  ...
  it("renders the endpoint page as a three-zone operations workbench", () => {
    useFilterStore.setState({
      search: "",
      enabledOnly: false,
      transformer: "all",
    ...
    const { getByTestId } = renderEndpoints();

    expect(getByTestId("endpoint-workbench")).toHaveClass("xl:grid-cols-[minmax(0,1fr)_360px]");
    expect(getByTestId("endpoint-workbench-main")).toHaveClass("min-w-0");
    expect(getByTestId("endpoint-workbench-inspector")).toHaveClass("xl:sticky");
  });
});
